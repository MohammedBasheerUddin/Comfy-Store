...
```html
<html lang="en" data-theme="fantasy"></html>
```
...
- Local Storage Theme Retrieval:
  ...
  - Create a function named `getThemeFromLocalStorage`.
    - Return the value of the `'theme'` key from `localStorage` or the default theme `'fantasy'`.

- Logic:

  - Create a state variable `theme` using the `useState` hook and initialize it with the result of `getThemeFromLocalStorage()`.
  - Define a function `handleTheme` that toggles between the `'fantasy'` and `'forest'` themes based on the current theme.
...
const Navbar = () => {
  ...
  const handleTheme = () => {
    const { fantasy, forest } = themes;
    const newTheme = theme === fantasy ? forest : fantasy;
    setTheme(newTheme);
  };
...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    toggleTheme: (state) => {
      const { forest, fantasy } = themes;
      state.theme = state.theme === forest ? fantasy : forest;
